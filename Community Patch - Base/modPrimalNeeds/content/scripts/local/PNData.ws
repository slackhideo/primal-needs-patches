function PNGetItemAttributeValue(att : SAbilityAttributeValue) : int { return RoundMath(att.valueAdditive); } //modPNPatches
function PNGetAttributeValue    (att : SAbilityAttributeValue) : int { return RoundMath(att.valueBase); }     //modPNPatches
...
function PNGetDrinkLevel( _inv : CInventoryComponent, itemId : SItemUniqueId) : int { return RoundMath((float)PNGetItemAttributeValue(_inv.GetItemAttributeValue( itemId, 'drink' )) * PNGetDrinkMult(_inv, itemId)); }
